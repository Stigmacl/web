...
const Ranking: React.FC = () => {
  ...
  const loadServerConfigs = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/servers/get-all.php?ranking_only=true`);
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/rankings/get-rankings.php?server_ip=${selectedServer.ip}&server_port=${selectedServer.port}&order_by=${orderBy}&limit=100`
      );
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="text-center py-12">
          ...
          <h2 className="text-3xl font-bold text-white mb-4">No hay servidores de ranking configurados</h2>
          ...
          return (
            <button
              ...
              className={`p-4 rounded-xl border-2 transition-all duration-300 ${
                selectedServer && selectedServer.ip === server.ip && selectedServer.port === server.port
