...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  // Limpiar token si se cierra sesión
  const clearRememberToken = () => {
    localStorage.removeItem('rememberToken');
  };

  // Guardar token en localStorage
  const saveRememberToken = (token: string) => {
  ...
  const checkExistingSession = async () => {
    try {
      ...
      let data;
      if (rememberToken) {
        data = await apiRequest('/auth/check-session.php', {
          method: 'POST',
  ...
  const login = async (username: string, password: string, remember: boolean = false): Promise<boolean> => {
    try {
      ...
      if (data.success) {
        ...
        if (remember && data.rememberToken) {
          saveRememberToken(data.rememberToken);
          console.log('💾 Token de recordar sesión guardado');
        }
  ...
  const updateUser = async (userId: string, updates: Partial<User>): Promise<boolean> => {
    try {
      ...
      if (data.success) {
        await loadUsers();
        if (userId === user?.id) {
          const usersData = await apiRequest('/users/get-all.php');
          if (usersData.success) {
            const updatedUser = usersData.users.find((u: User) => u.id === userId);
            if (updatedUser) {
              setUser(updatedUser);
            }
          }
