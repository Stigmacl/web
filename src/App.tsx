...
export type Section = 'home' | 'players' | 'servers' | 'download' | 'user-panel' | 'forum' | 'sponsors' | 'ranking';
    ...
    >
      ...
      <div className="fixed inset-0 z-0">
        <video
          ...
          muted
          playsInline
          className="w-full h-full object-cover video-background-blur"
        >
          <source src="/Fondo WEB Largo.mp4" type="video/mp4" />
        </video>
        <div className="absolute inset-0 bg-black/50"></div>
      </div>

      {/* Fallback gradient background if video doesn't load */}
      <div className={`fixed inset-0 z-0 bg-gradient-to-br ${themeConfig.colors.background} video-fallback`}></div>
      ...
      {isEnabled && bannerItems.length > 0 && (
        <div className="relative z-50 pt-4">
          <DynamicBanner bannerItems={bannerItems} isEnabled={isEnabled} />
        </div>
      )}

      {/* Navigation debajo del banner */}
      <div className={`relative z-50 ${isEnabled && bannerItems.length > 0 ? 'mt-4' : ''}`}>
        <Navigation currentSection={currentSection} onSectionChange={setCurrentSection} />
      </div>
      <SessionManager />
      <ThemeSelector />

      <main className={`relative z-20 ${isEnabled && bannerItems.length > 0 ? 'pt-8' : 'pt-24'}`}>
        {renderSection()}
      </main>

      <footer
        className="relative z-20 mt-20 py-8 border-t"
