// js/icons.js — registro único de íconos SVG (trazo 24x24) para toda la plataforma.
(function () {
    const P = {
        // Interfaz
        dashboard: '<rect x="3" y="3" width="7" height="9" rx="1.5"/><rect x="14" y="3" width="7" height="5" rx="1.5"/><rect x="14" y="12" width="7" height="9" rx="1.5"/><rect x="3" y="16" width="7" height="5" rx="1.5"/>',
        calendar: '<rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/>',
        wheel: '<circle cx="12" cy="12" r="9"/><path d="M12 12V3M12 12l7.8 4.5M12 12l-7.8 4.5"/><circle cx="12" cy="12" r="1.6"/>',
        ...
        gavel: '<path d="m14.5 12.5-8 8a2.12 2.12 0 1 1-3-3l8-8"/><path d="m16 16 6-6M8 8l6-6M9 7l8 8M21 11l-8-8"/>',
    ...
    // Íconos elegibles para premios en el panel admin (clave → etiqueta).
    const PRIZE_ICONS = {
        gift: 'Regalo', clock: 'Tiempo', sun: 'Día libre', home: 'Teletrabajo',
        utensils: 'Almuerzo', coffee: 'Café', headphones: 'Audio', laptop: 'Tecnología',
        ticket: 'Entradas', plane: 'Viaje', bag: 'Compras', card: 'Gift card',
        trophy: 'Trofeo', medal: 'Medalla', star: 'Destacado', heart: 'Bienestar', sparkles: 'Sorpresa',
    };

    function svg(name, cls = 'icon') {
        const body = P[name] || P.gift;
    ...
    // Los premios guardan "icon:<clave>" o, en datos antiguos, un emoji.
    function prize(icono, cls = 'icon') {
        const value = (icono || '').trim();
        if (value.startsWith('icon:') && P[value.slice(5)]) {
            return svg(value.slice(5), cls);
        }
        if (!value) return svg('gift', cls);
        const span = document.createElement('span');
        span.className = `prize-emoji ${cls}`;
    ...
    window.S360.icons = { svg, prize, PRIZE_ICONS };
})();
